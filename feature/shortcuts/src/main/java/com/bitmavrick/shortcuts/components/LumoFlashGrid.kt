...
fun LumoFlashGrid(
    ...
    LaunchedEffect(flashList) {
        ...
        list = flashList
        delay(100)
        onEvent(ShortcutUiEvent.ReorderPinnedFlash(list))
    }
        ...
        itemsIndexed(list, key = { _, item -> item.id }) { _, item ->
            ...
            ReorderableItem(reorderableLazyStaggeredGridState, item.id) {
                LumoFlashCard(
                    ...
                    onAddToHome = {
                        if(item.isPinned){
                            onEvent(ShortcutUiEvent.RemoveFromHome(item))
                        }else{
                            onEvent(ShortcutUiEvent.AddToHome(item))
                        }
                    },
            ...
            if(showUpdateDetailsBottomSheet){
                FlashDetailsBottomSheet(
                    ...
                    onUpdate = {
                        onEvent(ShortcutUiEvent.UpdateFlash(it))
                    },
                    onDismiss = {
            ...
            if(showDeleteDetailsBottomSheet){
                DeleteFlashDialog(
                    flash = item,
                    onDelete = {
                        onEvent(ShortcutUiEvent.DeleteFlash(it))
