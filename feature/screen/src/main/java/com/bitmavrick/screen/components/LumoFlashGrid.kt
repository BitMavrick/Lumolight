...
fun LumoFlashGrid(
    ...
    LaunchedEffect(flashList) {
        ...
        list = flashList
        delay(100)
        onEvent(ScreenFlashUiEvent.ReorderBothFlash(list))
    }
        ...
        itemsIndexed(list, key = { _, item -> item.id }) { _, item ->
            ...
            ReorderableItem(reorderableLazyStaggeredGridState, item.id) {
                LumoFlashCard(
                    ...
                    onAddToHome = {
                        if(item.isPinned){
                            onEvent(ScreenFlashUiEvent.RemoveFromHome(item))
                        }else{
                            onEvent(ScreenFlashUiEvent.AddToHome(item))
                        }
                    },
            ...
            if(showUpdateDetailsBottomSheet){
                FlashDetailsBottomSheet(
                    ...
                    onUpdate = {
                        onEvent(ScreenFlashUiEvent.UpdateFlash(it))
                    },
                    onDismiss = {
            ...
            if(showDeleteDetailsBottomSheet){
                DeleteFlashDialog(
                    flash = item,
                    onDelete = {
                        onEvent(ScreenFlashUiEvent.DeleteFlash(it))
